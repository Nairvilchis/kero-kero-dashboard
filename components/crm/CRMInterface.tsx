...
import { Search, User, Tag, Mail, Phone, Save, Loader2 } from 'lucide-react'
...
export default function CRMInterface({ instanceId }: CRMInterfaceProps) {
    ...
    useEffect(() => {
        ...
        const loadContacts = async () => {
            setLoading(true)
            try {
                const { data } = await crmApi.list(instanceId)
                setContacts(data || [])
            } catch (error) {
    ...
    const handleSaveContact = async (e: React.FormEvent) => {
        e.preventDefault()
        if (!selectedContact || !instanceId) return

        try {
            await crmApi.update(instanceId, selectedContact.jid, {
    ...
    return (
        <div className="flex h-[600px] bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
                    ...
                    {loading ? (
                        ...
                        filteredContacts.map((contact) => (
                            <button
                                ...
                                className={`w-full p-4 flex items-start gap-3 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors text-left border-b border-zinc-100 dark:border-zinc-800
            ...
            {selectedContact ? (
                <div className="flex-1 overflow-y-auto bg-white dark:bg-black p-8">
                    <div className="max-w-2xl mx-auto">
                        <div className="bg-zinc-50 dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800 overflow-hidden">
